...
from app.api.deps import get_email_service, require_admin_api_key, get_current_user_id
...
async def send_welcome(data: WelcomeEmailRequest, _=Depends(require_admin_api_key),
...
async def send_order_confirmation(data: OrderConfirmationRequest, _=Depends(require_admin_api_key),
                                   email: EmailService = Depends(get_email_service)):
...
async def send_shipping(data: ShippingNotificationRequest, _=Depends(require_admin_api_key),
                         email: EmailService = Depends(get_email_service)):
...
async def send_delivery(data: DeliveryConfirmationRequest, _=Depends(require_admin_api_key),
                         email: EmailService = Depends(get_email_service)):
...
async def send_payment_failed(data: PaymentFailedRequest, _=Depends(require_admin_api_key),
                               email: EmailService = Depends(get_email_service)):
...
async def send_refund(data: RefundConfirmationRequest, _=Depends(require_admin_api_key),
                       email: EmailService = Depends(get_email_service)):
...
async def send_cancelled(data: OrderCancelledRequest, _=Depends(require_admin_api_key),
                          email: EmailService = Depends(get_email_service)):
...
async def send_back_in_stock(data: BackInStockRequest, _=Depends(require_admin_api_key),
                              email: EmailService = Depends(get_email_service)):
...
async def send_abandoned_cart(data: AbandonedCartRequest, _=Depends(require_admin_api_key),
                               email: EmailService = Depends(get_email_service)):
...
async def send_new_drop(data: NewDropAlertRequest, _=Depends(require_admin_api_key),
                         email: EmailService = Depends(get_email_service)):
